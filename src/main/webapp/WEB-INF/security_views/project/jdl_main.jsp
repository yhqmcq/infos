...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $("#d1").datagrid({
			...
			remoteSort: false, toolbar: '#buttonbar', striped:true, pagination: true,pageSize: 500,pageList: [10,20,30,50,100,200,500,1500],
			frozenColumns: [[
	...
	//过滤生产部门稼动率
	function exportDept() {
		s1.combotree('setValue','');
		$("#d1").datagrid({
			url:yhq.basePath+"/project/jdl/employeeTaskTimeReport_leader.do?notInStatus=9999"+("${ViewType}" != ""?"&viewType=Y":"")+"&exportDept=all",
			onLoadSuccess: function(data) {
		        $.fn.datagrid.extensions.onLoadSuccess.apply(this, arguments);  //这句一定要加上。
				$("#jl_bar").show();
				$(dataGrid.datagrid('getColumnDom',"month"+(new Date().getMonth()+1))).css("background","#55BF3B");
